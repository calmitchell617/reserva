...
func main() {
	...
	flag.BoolVar(&cfg.smtp.enabled, "smtp-enabled", false, "SMTP enabled")
	...
	if cfg.smtp.enabled {
		if cfg.smtp.host == "" {
			fmt.Println("You must enter an SMTP host if SMTP is enabled")
			os.Exit(1)
		}
		if cfg.smtp.port == 0 {
			fmt.Println("You must enter an SMTP port if SMTP is enabled")
			os.Exit(1)
		}
		if cfg.smtp.username == "" {
			fmt.Println("You must enter an SMTP username if SMTP is enabled")
			os.Exit(1)
		}
		if cfg.smtp.password == "" {
			fmt.Println("You must enter an SMTP password if SMTP is enabled")
			os.Exit(1)
		}

		if cfg.smtp.sender == "" {
			fmt.Println("You must enter an SMTP sender if SMTP is enabled")
			os.Exit(1)
		}
	}
